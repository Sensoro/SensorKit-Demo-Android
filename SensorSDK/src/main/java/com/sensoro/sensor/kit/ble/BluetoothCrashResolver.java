...
class BluetoothCrashResolver {
    ...
    public void start() {
        ...
        Logger.debug(TAG, "started listening for BluetoothAdapter events");
    }
    ...
    public void stop() {
        ...
        Logger.debug(TAG, "stopped listening for BluetoothAdapter events");
        saveState();
    }
    ...
    public void crashDetected() {
        if (android.os.Build.VERSION.SDK_INT < 18) {
            Logger.debug(TAG, "Ignoring crashes before API 18, because BLE is unsupported.");
            return;
        }
        Logger.warn(TAG, "BluetoothService crash detected");
        if (distinctBluetoothAddresses.size() > 0) {
            Logger.debug(TAG, "Distinct Bluetooth devices seen at crash: %s" +
                    distinctBluetoothAddresses.size());
        }
        ...
        if (recoveryInProgress) {
            Logger.debug(TAG, "Ignoring Bluetooth crash because recovery is already in progress.");
        } else {
            startRecovery();
        }
        processStateChange();
    ...
    private void startRecovery() {
        ...
        Logger.debug(TAG, "about to check if discovery is active");
        if (!adapter.isDiscovering()) {
            Logger.warn(TAG, "Recovery attempt started");
            recoveryInProgress = true;
            discoveryStartConfirmed = false;
            Logger.debug(TAG, "about to command discovery");
            if (!adapter.startDiscovery()) {
                Logger.warn(TAG, "Can't start discovery.  Is Bluetooth turned on?");
            }
            Logger.debug(TAG, "startDiscovery commanded.  isDiscovering()=%s" + adapter.isDiscovering());
            ...
            if (TIME_TO_LET_DISCOVERY_RUN_MILLIS > 0) {
                Logger.debug(TAG, "We will be cancelling this discovery in %s milliseconds." + TIME_TO_LET_DISCOVERY_RUN_MILLIS);
                cancelDiscovery();
            } else {
                Logger.debug(TAG, "We will let this discovery run its course.");
            }
        } else {
            Logger.warn(TAG, "Already discovering.  Recovery attempt abandoned.");
        }

    }

    private void finishRecovery() {
        Logger.warn(TAG, "Recovery attempt finished");
        synchronized (distinctBluetoothAddresses) {
    ...
    private final BroadcastReceiver receiver = new BroadcastReceiver() {
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (action.equals(BluetoothAdapter.ACTION_DISCOVERY_FINISHED)) {
                if (recoveryInProgress) {
                    Logger.debug(TAG, "Bluetooth discovery finished");
                    finishRecovery();
                } else {
                    Logger.debug(TAG, "Bluetooth discovery finished (external)");
                }
            }
            if (action.equals(BluetoothAdapter.ACTION_DISCOVERY_STARTED)) {
                if (recoveryInProgress) {
                    discoveryStartConfirmed = true;
                    Logger.debug(TAG, "Bluetooth discovery started");
                } else {
                    Logger.debug(TAG, "Bluetooth discovery started (external)");
                }
            }
            ...
            if (action.equals(BluetoothAdapter.ACTION_STATE_CHANGED)) {
                ...
                switch (state) {
                    case BluetoothAdapter.ERROR:
                        Logger.debug(TAG, "Bluetooth state is ERROR");
                        break;
                    case BluetoothAdapter.STATE_OFF:
                        Logger.debug(TAG, "Bluetooth state is OFF");
                        lastBluetoothOffTime = System.currentTimeMillis();
                        break;
                    case BluetoothAdapter.STATE_TURNING_OFF:
                        break;
                    case BluetoothAdapter.STATE_ON:
                        Logger.debug(TAG, "Bluetooth state is ON");
                        Logger.debug(TAG, "Bluetooth was turned off for %s milliseconds" + (lastBluetoothTurningOnTime - lastBluetoothOffTime));
                    ...
                    case BluetoothAdapter.STATE_TURNING_ON:
                        ...
                        Logger.debug(TAG, "Bluetooth state is TURNING_ON");
                        break;
                }
            }
        }
    ...
    private void saveState() {
        ...
        try {
            ...
            Logger.warn(TAG, "Can't write macs to %s" + DISTINCT_BLUETOOTH_ADDRESSES_FILE);
        } finally {
            if (writer != null) {
                try {
                    writer.close();
        ...
        Logger.debug(TAG, "Wrote %s Bluetooth addresses" + distinctBluetoothAddresses.size());
    ...
    private void loadState() {
        ...
        try {
            ...
            Logger.warn(TAG, "Can't read macs from %s" + DISTINCT_BLUETOOTH_ADDRESSES_FILE);
        } catch (NumberFormatException e) {
            Logger.warn(TAG, "Can't parse file %s" + DISTINCT_BLUETOOTH_ADDRESSES_FILE);
        } finally {
            if (reader != null) {
                try {
                    reader.close();
        ...
        Logger.debug(TAG, "Read %s Bluetooth addresses" + distinctBluetoothAddresses.size());
    }
    ...
    private void cancelDiscovery() {
        try {
            ...
            if (!discoveryStartConfirmed) {
                Logger.warn(TAG, "BluetoothAdapter.ACTION_DISCOVERY_STARTED never received.  Recovery may fail.");
            }

            final BluetoothAdapter adapter = BluetoothAdapter.getDefaultAdapter();
            if (adapter.isDiscovering()) {
                Logger.debug(TAG, "Cancelling discovery");
                adapter.cancelDiscovery();
            } else {
                Logger.debug(TAG, "Discovery not running.  Won't cancel it");
            }
        } catch (InterruptedException e) {
            Logger.debug(TAG, "DiscoveryCanceller sleep interrupted.");
        }
    }
}
